  ...
  <div>
    ...
    <div v-if="selectedGroups.length > 0" class="mb-4 flex flex-wrap gap-2">
      <div v-for="groupId in selectedGroups" :key="groupId" class="bg-surface-700 px-3 py-1 rounded-full flex items-center gap-1">
        ...
        <button type="button" @click.stop="toggleGroupSelection(groupId)" class="flex items-center text-gray-400 hover:text-white duration-300">
    ...
    <div class="relative" ref="groupSelectorRef">
      <button type="button" @click.stop="showGroupSelector = !showGroupSelector" class="w-full p-3 bg-surface-950 border-2 border-transparent rounded-xl hover:border-primary-500 focus:border-primary-500 outline-none transition-colors text-left flex justify-between items-center">
      ...
      <div v-if="showGroupSelector" class="absolute z-10 mt-2 w-full border-2 border-surface-700 rounded-xl p-4 bg-surface-900 space-y-4 shadow-xl">
        <div class="relative">
          <input v-model="searchQuery" type="text" class="w-full p-2 pl-8 bg-surface-950 border border-surface-700 rounded-lg focus:border-primary-500 outline-none" placeholder="Search groups..." />
        ...
        <div v-else class="max-h-64 overflow-y-auto space-y-2">
          <div v-for="group in filteredGroups" :key="group.id" class="flex items-center justify-between p-2 hover:bg-surface-800 rounded-lg cursor-pointer transition-colors" @click="toggleGroupSelection(group.id)">
            ...
            <div class="w-5 h-5 rounded border flex items-center justify-center" :class="selectedGroups.includes(group.id) ? 'bg-primary-500 border-primary-500' : 'border-gray-500'">
              <Icon v-if="selectedGroups.includes(group.id)" name="material-symbols:check" size="0.875rem" class="text-white" />
            </div>
          </div>
        </div>
      </div>
