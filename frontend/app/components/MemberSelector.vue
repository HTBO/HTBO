  ...
  <div>
    ...
    <div v-if="selectedMembers.length > 0" class="mb-4 flex flex-wrap gap-2">
      <div v-for="memberId in selectedMembers" :key="memberId" class="bg-surface-700 px-3 py-1 rounded-full flex items-center gap-1">
        ...
        <button type="button" @click.stop="toggleMemberSelection(memberId)" class="flex items-center text-gray-400 hover:text-white duration-300">
    ...
    <div class="relative" ref="memberSelectorRef">
      <button type="button" @click.stop="showMemberSelector = !showMemberSelector" class="w-full p-3 bg-surface-950 border-2 border-transparent rounded-xl hover:border-primary-500 focus:border-primary-500 outline-none transition-colors text-left flex justify-between items-center">
          ...
          <div v-for="user in filteredFriends" :key="user._id" class="flex items-center justify-between p-2 hover:bg-surface-800 rounded-lg cursor-pointer transition-colors" @click="toggleMemberSelection(user._id)">
            <div class="flex items-center gap-3">
              <div class="size-8 bg-surface-600 rounded-full flex items-center justify-center">
                <NuxtImg :src="user.avatarUrl" class="rounded-full" width="32" height="32" />
              </div>
