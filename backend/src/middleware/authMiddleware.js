...
async function verifyToken(req, res, next) {
    ...
    try {        
        ...
        if (blacklisted) return res.status(401).json({ error: 'Token revoked | ERRC: 03' });
...
function checkUserPermission(req, res, next) {
    if (req.params.id !== String(req.userId)) {
        return res.status(403).json({ error: 'Unauthorized access | ERRC: 10' });
    }
    next();
}
...
async function checkSessionPermission (req, res, next) {
    ...
    if(req.userId !== String(hostId.hostId)) {
        return res.status(403).json({ error: 'Unauthorized access | ERRC: 20' });
    }
    next();
}
...
async function checkGroupPermission (req, res, next) {
    ...
    if(req.userId !== String(ownerId.ownerId)) {
        return res.status(403).json({ error: 'Unauthorized access | ERRC: 40' });
    }
    next();
}
