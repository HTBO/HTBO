...
describe("Login process", () => {
    ...
    test('Login with invalid username and password fails', async () => {
        ...
        expect(login.body.error).toBe("Username or password does not match | ERRC:0")
    }),
    test('Login with invalid email and password fails', async () => {
        ...
        expect(login.body.error).toBe("Email or password does not match | ERRC:1")
    }),
    test('Login with username and invalid password fails', async () => {
        ...
        expect(login.body.error).toBe("Username or password does not match | ERRC:2")
    }),
    test('Login with email and invalid password fails', async () => {
        ...
        expect(login.body.error).toBe("Email or password does not match | ERRC:2")
    }),
    test('Login without username or email and password fails', async () => {
        login = await request(app).post('/api/users/login').send({
            "password": "joe"
        })
        expect(login.body.error).toBe("Please provide an email or username | ERRC:01")
    }),
    test('Login with username and without password fails', async () => {
        login = await request(app).post('/api/users/login').send({
            "username": "JoeBiden"
        })
        expect(login.body.error).toBe("Please provide the password | ERRC:2")
    }),
